...
public class OrganisationElementDefinition {
  ...
  public static final FieldMetadataExtended<OrganisationElementFull, OrganisationElementFullWritable, String> OWNER_METADATA = FieldMetadataExtended
    .<OrganisationElementFull, OrganisationElementFullWritable, String>builder().setDisplayedName("owner")
    .setValueGetter(element -> element.getMetadata().owner)
    .setValueSetter((element, owner) -> element.getMetadata().owner = owner)
    .addTag(Tag.PRESERVE)
    .addTag(Tag.SKIP_INTERACTIVE_INPUT)
    .setValueBuilder(new StringFactory())
    .setSQLReader(resultSet -> resultSet.getString("owner"))
    .build();
  ...
  public static class OrganisationElementAutogenerated implements Serializable {
    ...
    protected @Nullable String owner = null;
