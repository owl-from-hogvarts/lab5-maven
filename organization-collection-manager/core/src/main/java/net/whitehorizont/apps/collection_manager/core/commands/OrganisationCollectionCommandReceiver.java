...
public class OrganisationCollectionCommandReceiver extends CollectionCommandReceiver<OrganisationElementFull> implements IOrganisationCollectionCommandReceiver {
  ...
  public void removeByRevenue(RemovalCriteria removalCriteria, double targetValue) {
    final var keysToDelete = this.getEveryWithKey$().filter(keyElement -> {
      final @NonNull var currentAnnualTurnover = OrganisationElementDefinition.ANNUAL_TURNOVER_METADATA.getValueGetter()
          .apply(keyElement.getValue().getElement());
  ...
  @Override
  public Observable<Entry<ElementKey, OrganisationElementFull>> getStartsWith$(String startOfFullName) {
    return collection.getEveryWithKey$().filter(keyElement -> OrganisationElementDefinition.NAME_METADATA
        .getValueGetter().apply(keyElement.getValue().getElement()).startsWith(startOfFullName));
  }

  @Override
  public Observable<Entry<ElementKey, OrganisationElementFull>> getDescending$() {
